...
export function PrintSheet({
    ...
    const qr = (unit?: number) => {
        const token = unit
            ? project.units.find((u) => u.id === unit)?.public_token
            : project.public_token;
        return `/api/qr-code?surface=production&size=512&path=${encodeURIComponent(token ? `/production/label?token=${token}` : `/production?project=${project.project_id}${unit ? `&unit=${unit}#unit-${unit}` : ''}`)}`;
    };
    return (
        <div className={styles.printSheets} aria-hidden="true">
            {(project.flow_version !== 2 ||
                ...
                <section>
                    ...
                    <p>
                        {project.units_count} вещей · проект #
                        {project.project_id}
                    </p>
                    ...
                    />
                    <p>
                        Один заказ — один мешок. Проверяйте актуальное состояние
                        в терминале.
                    </p>
                    <ul>
                        ...
                        <section key={unit.id}>
                            ...
                            {unit.specification ? (
                                <>
                                    ...
                                    <ul>
                                        {unit.specification.components.map(
                                            (x) => (
                                                <li key={x.key}>
                                                    □ {x.name} · {x.quantity}{' '}
                                                    {x.unit} · {x.location}
