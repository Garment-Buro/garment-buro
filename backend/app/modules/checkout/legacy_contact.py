...
async def prepare_legacy_contact_order(
    ...
    *,
    ...
    recipient: CheckoutContact | None,
    database: DatabaseManager | None,
) -> dict:
    if database is None or not database.enabled:
        raise HTTPException(503, "Личный кабинет временно недоступен")
    if (
        not str(payload.get("delivery_city") or "").strip()
        or not str(payload.get("delivery_address") or "").strip()
    ):
    ...
    if method == "cdek_pickup":
        async with database.session() as session:
            ...
            if (
                ...
                or (datetime.now(UTC) - state.updated_at.replace(tzinfo=UTC) > timedelta(days=7))
            ):
