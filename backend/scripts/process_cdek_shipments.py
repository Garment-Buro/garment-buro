...
from app.modules.delivery.worker import CdekShipmentProcessor

logger = logging.getLogger(__name__)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Create durable CDEK shipments")
    parser.add_argument(
        "--once",
        action="store_true",
...
async def process_batch(
    ...
    *,
    ...
    for _ in range(max_items):
        ...
        processed += 1
        logger.info(
            "CDEK shipment %s finished attempt %s with status %s error %s",
            result.shipment_id,
...
async def run(args: argparse.Namespace) -> None:
    ...
    if not settings.cdek_creation_enabled:
        raise RuntimeError("CDEK_CREATION_ENABLED is required")
    poll_seconds = (
        args.poll_seconds if args.poll_seconds is not None else float(settings.cdek_poll_seconds)
    )
    if poll_seconds <= 0:
    ...
    transport = AiohttpCdekTransport(settings)
    processor = CdekShipmentProcessor(
        settings,
