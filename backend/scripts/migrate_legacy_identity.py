...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Audit or import legacy users without carrying plaintext OTP state",
    )
...
async def apply_plan(plan: IdentityMigrationPlan) -> dict[str, object]:
    ...
    return {
        ...
        "counts": {"users": result.users},
    }


def main() -> int:
    args = parse_args()
    plan = LegacyIdentityPlanner().build(args.sqlite_db)
    report = plan.report()
    exit_code = 0 if plan.valid else 1
    if args.apply:
        if not plan.valid:
            report["apply_error"] = "Identity migration plan is invalid"
            exit_code = 2
